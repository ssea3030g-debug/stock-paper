...
두 개의 예약 작업이 있습니다.
- **매일 06:40 KST**: 신문 전체를 새로 만듭니다 (미국 증시 종가 확정 뒤, 7시 전 완성).
...
5. 비어 있지 않으면, 위 "매일 아침 지시문"의 4~10번을 그대로 실행해 (holdings 는 이미 3번에서 받았으니 다시 받지 않아도 됨). 커밋 메시지는 `사이트: <오늘> 호 (새 종목 반영)` 으로 해.
